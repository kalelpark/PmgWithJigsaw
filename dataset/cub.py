...
root = "dataset/data/cubbird/CUB_200_2011/"

def cub_loader(args):
    train_dataset = ImageLoader(root, transform = get_transform(args), train = True)
    valid_dataset = ImageLoader(root, train = False, transform = get_transform(args, train = False))

    train_loader = DataLoader(train_dataset, batch_size = args.batchsize, shuffle = True, pin_memory = True)
    test_loader = DataLoader(valid_dataset, batch_size = args.batchsize, shuffle = False, pin_memory = True)
